...
此功能为 AstrBot 移植版新增，不算入原版 53 条路由。未匹配现有解析规则的公开网页自动截图，已支持的平台和文章总结优先；解析失败不触发截图。默认宽度 1280、高度上限 6000，可配置开关/超时/高度。独立匿名会话，公开 GET 资源，JS/CSS 可渲染；登录、POST 数据、验证码页面不保证完整显示。
