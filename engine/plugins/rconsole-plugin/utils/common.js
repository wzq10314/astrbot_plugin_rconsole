...
import { exec } from "../../../safe-process.mjs";
...
export async function downloadAudio(mp3Url, filePath, title = "temp", redirect = "manual", audioType = "mp3") {
    title = String(title).replace(/[\\/\x00-\x1f<>:"|?*]/g, '_').slice(0, 150) || 'audio';
    audioType = /^[a-z0-9]{2,5}$/i.test(audioType) ? audioType : 'mp3';
    // 如果没有目录就创建一个
...
export function checkToolInCurEnv(someCommand) {
    return globalThis.rc.call('which', {name: someCommand});
}
