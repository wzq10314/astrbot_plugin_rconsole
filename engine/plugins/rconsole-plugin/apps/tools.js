...
import { exec, execSync } from "../../../safe-process.mjs";
...
export class tools extends plugin {
    ...
    async bili(e) {
        ...
        // AstrBot: extract a URL from share text for mobile/dynamic links too.
    ...
    async biliMusic(e, url) {
        ...
        await this.queue.add(async () => {
            await getBiliAudio(videoId, "").then(async audioUrl => {
                const path = this.getCurDownloadPath(e);
    ...
    async acfun(e) {
        ...
        const res = await parseUrl(inputMsg);
        await e.reply(`${this.identifyPrefix}识别：猴山，${res.videoName}`);
        const res2 = await parseM3u8(res.urlM3u8s[res.urlM3u8s.length - 1]);
        await downloadM3u8Videos(res2.m3u8FullUrls, path);
        await mergeAcFileToMp4(res2.tsNames, path, `${path}out.mp4`);
        await this.sendVideoToUpload(e, `${path}out.mp4`);
        return true;
    }
    ...
    async netease(e) {
        ...
        if (/\/program\?|\/dj\?/.test(message)) {
            ...
            // 下载音乐
            await downloadAudio(url, this.getCurDownloadPath(e), title, 'follow', musicExt).then(async path => {
                // 发送群文件
    ...
    async freyr(e) {
        ...
        if (!isOversea) {
            ...
            // 下载音乐
            await downloadAudio(url, this.getCurDownloadPath(e), title, 'follow').then(async path => {
                // 发送语音
                if (this.isSendVocal) {
                    await e.reply(segment.record(path));
                }
    ...
    async linkShareSummary(e) {
        ...
        // AstrBot adapter uses the current conversation's provider; no duplicate API key.
        return await this._generalLinkShareSummary(e, summaryLink, name);
    }
